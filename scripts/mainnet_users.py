...
def get_balance_history(display_address):
    ...
    url = "https://alephzero.api.subscan.io/api/v2/scan/transfers"

    payload = json.dumps({
        "address": display_address,
        "asset_symbol": "AZERO",
        "asset_unique_id": "AZERO",
        "order": "asc",
        "page": 0,
        "row": 1,
        "success": True
...
def main():
    input_file = 'AZERO_holders.csv'  # Replace with your input CSV file name
    output_file = 'unique_users_on_the_mainnet.csv'  # Output file to save results
    ...
    with open(input_file, mode='r') as infile, open(output_file, mode='w', newline='', encoding='utf-8') as outfile:
        ...
        for row in reader:
            ...
            if balance_data and balance_data['code'] == 0:
                ...
                if transfers:  # Check if there are any transfers
                    first_block_timestamp = transfers[0]['block_timestamp']
    ...
    print("Data has been processed and saved to unique_users_on_the_mainnet.csv.")
